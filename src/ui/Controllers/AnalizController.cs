    ...
    ILogger<AnalizController> logger) : Controller
{
        ...
        [FromForm] string? ekNotlar)
    {
        ...
        {
            logger.LogError(ex, "Semptom analiz girdileri sayısal formata dönüştürülürken hata oluştu.");
            TempData["Hata"] = "Gönderilen semptom verileri geçersiz formatta.";
            return RedirectToAction("Yeni");
        }
        catch (Exception ex)
        {
            logger.LogError(ex, "Semptom analizi gerçekleştirilirken beklenmeyen bir hata oluştu.");
            TempData["Hata"] = "Analiz işlemi sırasında teknik bir hata oluştu. Lütfen tekrar deneyiniz.";
            return RedirectToAction("Yeni");
        }
    }
    ...
    public async Task<IActionResult> Sonuc(int id)
    {
        ...
        {
            logger.LogError(ex, "Analiz sonucu yüklenirken hata oluştu. AnalizSonucId: {AnalizSonucId}", id);
            TempData["Hata"] = "Analiz sonucu yüklenirken bir hata oluştu.";
            return RedirectToAction("Yeni");
        }
    }
}
