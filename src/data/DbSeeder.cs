...
public static class DbSeeder
{
        ...
        bool seedDemoUsers = false)
    {
        ...
        if (await userManager.FindByEmailAsync("admin@semptomanaliz.com") == null)
        {
            ...
            var r = await userManager.CreateAsync(admin, etkinAdminSifresi);
        ...
        // Semptom.Id 1-34, SemptomKatalog.Id 1-34 ile birebir eşleşir (aynı sırada eklendi)
        ...
        if (!await context.Hastaliklar.AnyAsync())
        {
            ...
            var iliskiler = new List<HastalikSemptom>
            {
                ...
                new() { HastalikId = bronsit.Id,  SemptomId = 32, Agirlik = 5  }, // Göğüs ağrısı
                ...
                new() { HastalikId = pnomoni.Id,  SemptomId = 32, Agirlik = 7  }, // Göğüs ağrısı
                ...
                new() { HastalikId = astim.Id,    SemptomId = 32, Agirlik = 5  }, // Göğüs sıkışması
                ...
                new() { HastalikId = kardiyak.Id, SemptomId = 32, Agirlik = 10 }, // Göğüs ağrısı
                new() { HastalikId = kardiyak.Id, SemptomId = 34, Agirlik = 8  }, // Baygınlık
                new() { HastalikId = kardiyak.Id, SemptomId = 7,  Agirlik = 8  }, // Nefes darlığı
                new() { HastalikId = kardiyak.Id, SemptomId = 33, Agirlik = 7  }, // Çarpıntı
                ...
                // ANJİN
                new() { HastalikId = anjin.Id,    SemptomId = 32, Agirlik = 10 }, // Göğüs ağrısı
                new() { HastalikId = anjin.Id,    SemptomId = 7,  Agirlik = 7  }, // Nefes darlığı
                new() { HastalikId = anjin.Id,    SemptomId = 34, Agirlik = 6  }, // Baygınlık
                new() { HastalikId = anjin.Id,    SemptomId = 24, Agirlik = 5  }, // Terleme
                new() { HastalikId = anjin.Id,    SemptomId = 33, Agirlik = 4  }, // Çarpıntı

                // ARİTMİ
                new() { HastalikId = aritmi.Id,   SemptomId = 33, Agirlik = 10 }, // Çarpıntı
                new() { HastalikId = aritmi.Id,   SemptomId = 34, Agirlik = 7  }, // Baygınlık
                new() { HastalikId = aritmi.Id,   SemptomId = 7,  Agirlik = 6  }, // Nefes darlığı
                new() { HastalikId = aritmi.Id,   SemptomId = 32, Agirlik = 5  }, // Göğüs ağrısı
                ...
                new() { HastalikId = htkriz.Id,   SemptomId = 32, Agirlik = 6  }, // Göğüs ağrısı
                new() { HastalikId = htkriz.Id,   SemptomId = 33, Agirlik = 4  }, // Çarpıntı
                ...
                // GERD
                new() { HastalikId = gerd.Id,     SemptomId = 32, Agirlik = 8  }, // Göğüs yanması
                ...
                new() { HastalikId = inme.Id,     SemptomId = 34, Agirlik = 8  }, // Baygınlık
                ...
                new() { HastalikId = anemi.Id,    SemptomId = 33, Agirlik = 6  }, // Çarpıntı
                ...
                // HİPERTİROİDİZM
                new() { HastalikId = hipertir.Id, SemptomId = 33, Agirlik = 9  }, // Çarpıntı
                ...
                // ANKSİYETE
                new() { HastalikId = anksiye.Id,  SemptomId = 33, Agirlik = 8  }, // Çarpıntı
                ...
                // PANİK ATAK
                new() { HastalikId = panik.Id,    SemptomId = 33, Agirlik = 10 }, // Çarpıntı
                new() { HastalikId = panik.Id,    SemptomId = 7,  Agirlik = 9  }, // Nefes darlığı
                new() { HastalikId = panik.Id,    SemptomId = 34, Agirlik = 9  }, // Baygınlık
                new() { HastalikId = panik.Id,    SemptomId = 24, Agirlik = 8  }, // Terleme
                new() { HastalikId = panik.Id,    SemptomId = 23, Agirlik = 7  }, // Titreme
                new() { HastalikId = panik.Id,    SemptomId = 32, Agirlik = 7  }, // Göğüs sıkışması
        ...
        await OnarKalpSemptomIliskileriAsync(context);
    ...
    private static async Task OnarKalpSemptomIliskileriAsync(AppDbContext context)
    {
        var semptomAdlari = new[]
        {
            "Kurdeşen",
            "Deride soyulma / kuruluk",
            "Göğüs ağrısı",
            "Çarpıntı",
            "Baygınlık hissi"
        };

        var semptomMap = await context.Semptomlar
            .Where(s => semptomAdlari.Contains(s.Ad))
            .ToDictionaryAsync(s => s.Ad, s => s.Id);

        if (!semptomMap.TryGetValue("Kurdeşen", out var kurdesenId) ||
            !semptomMap.TryGetValue("Deride soyulma / kuruluk", out var soyulmaId) ||
            !semptomMap.TryGetValue("Göğüs ağrısı", out var gogusId) ||
            !semptomMap.TryGetValue("Çarpıntı", out var carpintiId) ||
            !semptomMap.TryGetValue("Baygınlık hissi", out var bayginlikId))
        {
            return;
        }

        var dogruIliskiler = new List<(string HastalikAd, int SemptomId, int Agirlik)>
        {
            ("Bronşit", gogusId, 5),
            ("Pnömoni (Zatürre)", gogusId, 7),
            ("Astım", gogusId, 5),
            ("Kardiyak Değerlendirme Gerekli", gogusId, 10),
            ("Kardiyak Değerlendirme Gerekli", bayginlikId, 8),
            ("Kardiyak Değerlendirme Gerekli", carpintiId, 7),
            ("Anjin Pektoris", gogusId, 10),
            ("Anjin Pektoris", bayginlikId, 6),
            ("Anjin Pektoris", carpintiId, 4),
            ("Aritmisi / Çarpıntı Bozukluğu", carpintiId, 10),
            ("Aritmisi / Çarpıntı Bozukluğu", bayginlikId, 7),
            ("Aritmisi / Çarpıntı Bozukluğu", gogusId, 5),
            ("Hipertansif Kriz", gogusId, 6),
            ("Hipertansif Kriz", carpintiId, 4),
            ("GERD (Gastroözofajeal Reflü)", gogusId, 8),
            ("İnme (Serebrovasküler Olay)", bayginlikId, 8),
            ("Anemi (Kansızlık)", carpintiId, 6),
            ("Hipertiroidizm", carpintiId, 9),
            ("Anksiyete Bozukluğu", carpintiId, 8),
            ("Panik Atak", carpintiId, 10),
            ("Panik Atak", bayginlikId, 9),
            ("Panik Atak", gogusId, 7)
        };

        var hastalikAdlari = dogruIliskiler
            .Select(i => i.HastalikAd)
            .Distinct()
            .ToList();

        var hastalikMap = await context.Hastaliklar
            .Where(h => hastalikAdlari.Contains(h.Ad))
            .ToDictionaryAsync(h => h.Ad, h => h.Id);

        if (hastalikMap.Count == 0) return;

        var hastalikIdleri = hastalikMap.Values.ToList();
        var yanlisEskiSemptomIdleri = new[] { kurdesenId, soyulmaId, gogusId };
        var temizlenecekIliskiler = await context.HastalikSemptomlar
            .Where(hs => hastalikIdleri.Contains(hs.HastalikId)
                      && yanlisEskiSemptomIdleri.Contains(hs.SemptomId))
            .ToListAsync();

        if (temizlenecekIliskiler.Count > 0)
        {
            context.HastalikSemptomlar.RemoveRange(temizlenecekIliskiler);
            await context.SaveChangesAsync();
        }

        foreach (var (hastalikAd, semptomId, agirlik) in dogruIliskiler)
        {
            if (!hastalikMap.TryGetValue(hastalikAd, out var hastalikId)) continue;

            var mevcut = await context.HastalikSemptomlar
                .FirstOrDefaultAsync(hs => hs.HastalikId == hastalikId && hs.SemptomId == semptomId);

            if (mevcut == null)
            {
                context.HastalikSemptomlar.Add(new HastalikSemptom
                {
        ...
        string? kronik = null)
    {
        ...
        var analizGruplari = new List<(int[] semptomIdler, int gunOncesi, string? not)>
        {
            ...
            (new[]{17,18,19},   45, "Spor sonrası kas ağrıları."),
        ...
        HesaplaTestSonuc(int[] semptomlar, decimal bmi, string? kronik, int tekrar)
    {
        ...
        // Kas/eklem grubu (Eklem=17, Kas=18, Sırt=19)
        if (semptomlar.Contains(17) || semptomlar.Contains(18))
