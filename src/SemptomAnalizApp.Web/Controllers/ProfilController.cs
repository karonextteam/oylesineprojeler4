    ...
    ILogger<ProfilController> logger) : Controller
{
    ...
    public async Task<IActionResult> Index(ProfilViewModel model)
    {
        ...
        {
            logger.LogError(ex, "Sağlık profili kaydedilirken hata oluştu.");
            ModelState.AddModelError(string.Empty, "Profil kaydedilirken beklenmeyen bir hata oluştu. Lütfen tekrar deneyiniz.");
            return View(model);
        }
    }
